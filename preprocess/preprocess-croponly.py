...
cropped_img_path = '../input/data/croponly_train/'
try: 
...
cropped_eval_img_path = '../input/data/croponly_eval/'
try: 
...
with torch.no_grad():
    ...
    for i in tqdm(range(len(train_csv))):
        ...
        for filepath in files:
            ...
            #### 이렇게하면 얼굴 가로세로 비율때문에 찌그러짐 ####
            """
            crop_width  = b[BBoxX2] - b[BBoxX1]
            crop_height = b[BBoxY2] - b[BBoxY1]
            x_offset = int((crop_width  * 0.33) / 2)
            y_offset = int((crop_height * 0.33) / 2)

            cx1 = max(0, b[BBoxX1] - x_offset)
            cx2 = min(img_width - 1, b[BBoxX2] + x_offset)
            cy1 = max(0, b[BBoxY1] - y_offset)
            cy2 = min(img_height - 1, b[BBoxY2] + y_offset)
            """

            crop_width  = b[BBoxX2] - b[BBoxX1]
            crop_height = b[BBoxY2] - b[BBoxY1]
            center_pos = (int(crop_width / 2) + b[BBoxX1], int(crop_height / 2) + b[BBoxY1])
            crop_offset = int(max(int(crop_width / 2), int(crop_height / 2)) * 1.36) #### crop 배율

            cx1 = max(0, center_pos[0] - crop_offset)
            cx2 = min(img_width - 1, center_pos[0] + crop_offset)
            cy1 = max(0, center_pos[1] - crop_offset)
            cy2 = min(img_height - 1, center_pos[1] + crop_offset)

            ####
            img_raw = img_raw[cy1:cy2, cx1:cx2]
            align_crop_img = cv2.resize(img_raw, (384, 384), interpolation=cv2.INTER_CUBIC)
